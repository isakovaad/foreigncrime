...
// Manually defined label positions (based on visual inspection and adjustment)
const manualLabelPositions = {
    "Île-de-France": [[550, 200], [510, 200], [530, 190], [530, 240], [550, 220], [490, 220], [520, 220], [480, 190]],
    "Auvergne-Rhône-Alpes": [[680, 410], [600, 490], [550, 470], [680, 480], [640, 450], [720, 450], [550, 430], [600, 420], [650, 500]],
    "Normandie": [[400, 210], [430, 140], [380, 180], [340, 180], [420, 180]],
    "Bretagne": [[250, 235], [175, 240], [300, 240], [250, 270]],
    "Grand Est": [[620, 145], [610, 240], [600, 192], [665, 255], [670, 210], [675, 165], [720, 200], [770, 195], [750, 250]],
...
Promise.all([
    ...
    function renderRegionLabels() {
        ...
        svg.selectAll(".region-label")
            .data(Object.entries(manualLabelPositions).flatMap(([region, labels]) => 
                labels.map((position, index) => ({
                    region,
                    zip: regionMapping[region][index].split("-")[0], // Extract ZIP code
                    title: regionMapping[region][index], // Full title (e.g., "75-Paris")
                    x: position[0],
                    y: position[1]
                }))
            ))
            ...
            .attr("text-anchor", "middle")
            .text(d => d.zip) // Display ZIP code
            .attr("title", d => d.title) // Add full title for hover
            .style("font-size", d => d.region === "Île-de-France" ? "14px" : "18px") // Larger font size for Île-de-France
            .style("font-weight", "bold")
            .style("fill", d => d.region === "Île-de-France" ? "white" : "black") // Blue color for Île-de-France
            .style("pointer-events", "all") // Enable hover and interaction
            .on("mouseover", function (event, d) {
                d3.select(this)
                    .style("fill", "red") // Highlight on hover
            ...
            .on("mouseout", function () {
                // Reset styles and remove tooltip
    ...
    function renderRegionMaps() {
        ...
        geojson.features.forEach(feature => {
            ...
            zipCodes.forEach(zip => {
                const li = ul.append("li").text(zip);

                li.on("click", () => {
                    renderPieChart(regionName, zip);
                });
            });

            const pieChartContainer = regionContainer.append("div")
                .attr("class", "pie-chart")
                .attr("id", `pie-${regionName.replace(/\s+/g, "-")}`);
        });

        // Default Pie Chart for "75-Paris"
        renderPieChart("Île-de-France", "75-Paris");
    }

    function renderPieChart(regionName, zipCode) {
        const dataForRegion = filteredData.filter(d => d.Zone_geographique.includes(zipCode));
        createPieChart(dataForRegion, `#pie-${regionName.replace(/\s+/g, "-")}`);
    }
    ...
    function createPieChart(data, containerId) {
        ...
        const pie = d3.pie().value(d => d.Valeurs);
        ...
        svg.selectAll("path")
            ...
            .append("path")
            .attr("d", arc)
            .attr("fill", d => color(d.data.Indicateur))
            .append("title")
            .text(d => `${d.data.Indicateur}: ${d.data.Valeurs}`);
    }
